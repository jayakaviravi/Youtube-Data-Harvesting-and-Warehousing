...
def get_playlist_information(channel_id):
    ...
    nextpagetoken=None
    while True:
            response = youtube.playlists().list(part='snippet,contentDetails',channelId=channel_id,maxResults=100,pageToken=nextpagetoken).execute()
            ...
            for i in response['items']:
                    data=dict(channel_id=i['snippet']['channelId'],
                            ...
                            playlist_published=i['snippet']['publishedAt'].replace('Z',''),
                            video_count=int(i['contentDetails']['itemCount']))
...
#Create database in Mongo DB:
        ...
        if st.button(":blue[Upload datas to MongoDB]"):
            ...
            if ch_id in channel_ids:
                st.warning('The given channel id is already exists',icon='🚨')
            
            else:
                        
                c_details=get_Channel_information(ch_id)
                p_details=get_playlist_information(ch_id)
                v_id_details=get_channel_videos_ids(ch_id)
                v_details=get_video_information(v_id_details)
                com_details=get_comment_information(v_id_details)

                collection1=db["Channel_details"]
                collection1.insert_one({'channel_information':c_details,'playlist_information':p_details,
    ...
    with tab6:
        ...
        selected_channel=st.selectbox("select channel name",options=ch_name,index=None,placeholder="select channel name")

        result = collection1.find_one({"channel_information.channel_name":selected_channel})
...
# Function to create channel table:
        def insert_channel_table():
            ...
            try:
                ...
                print('channel table not created')
            

            channel_list=[]
            channel_list.append(result["channel_information"])
            df_channel=pd.DataFrame(channel_list)


            for index,row in df_channel.iterrows():
                channel_query='''insert into channels(channel_name,channel_id,channel_description,channel_views,channel_subcribers,
                                                        channel_total_videos,channel_published,channel_playlist_id,channel_country)
            ...
            for i in range(0,len(df_channel)):
                cur.execute(channel_query,tuple(df_channel.iloc[i]))
                mydb.commit()

            

# Function to create playlist table:

        def insert_playlist_table():
            ...
            try:
                create_playlist_table=''' create table if not exists playlists (channel_id varchar(225),
                                                                                channel_title varchar(225),
                ...
                print('playlist table not created')

            playlist_list = []
            for i in range(len(result['playlist_information'])):
                playlist_list.append(result['playlist_information'][i])
            df_playlist=pd.DataFrame(playlist_list)
            

            for i,row in df_playlist.iterrows():
                playlist_query='''insert into youtube_data.playlists values(%s,%s,%s,%s,%s,%s)'''
                cur.execute(playlist_query,tuple(row))
                mydb.commit()
...
#Function to create Video table:
        ...
        def insert_video_table ():
            ...
            try:
                create_video_table='''create table if not exists videos(channel_name varchar(100),
                                                                        channel_id varchar(100),
                                                                        v_id varchar(50) primary key,
                                                                        v_title varchar(100),
                                                                        v_description text,
                                                                        v_published timestamp,
                                                                        v_thumbnails varchar(100),
                                                                        v_tags text,
                                                                        v_duration time,
                                                                        v_caption varchar(50),
                                                                        v_definition varchar(30),
                                                                        v_views bigint,
                                                                        v_likes bigint,
                                                                        v_comments bigint
                                                                        )'''
                ...
                print('video table not created')


            video_list=[]
            for i in range(len(result['video_information'])):
                video_list.append(result["video_information"][i])
...
#Function to get comments table: 
        def insert_comment_table():
            ...
            try:
                    create_comment_table='''create table if not exists comments(v_id varchar(100),
                                                                                com_id varchar(50) primary key,
                                                                                com_text text,
                                                                                com_author varchar(100),
            ...
            comment_list=[]
            for i in range(len(result['comment_information'])):
                comment_list.append(result["comment_information"][i])
        ...
        if st.button(':blue[Migrate datas to My SQL]'):
            try:
                ...
                st.error('The given channel details already  transformed',icon='🚨')      
